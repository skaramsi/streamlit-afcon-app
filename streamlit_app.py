...
from fonctions import assists
from fonctions import assists ,player, passnetwork, passe, shot, pass_cross, pressure_heatmap, defensive_actions , mistake
...
teams_choice= st.selectbox('Team',teams)

def load_data(team_choice):
    mask = ((matches.home_team_name == team_choice) | (matches.away_team_name == team_choice) )
    games_selected = matches.loc[mask, ['match', 'match_date', 'kick_off','home_score','away_score',
                                        'stadium_name'
                                        ]]
...
plot_options= ['Passing Network', 'Passes', 'Pressure heatmap', 'Shots', 'Forward Passes','Crosses', 'Defensive Actions',
               'Mistakes','Assists','Player Performance']
...
if match_choice: 
    ...
    def event_data(match_choice):
        matchh = matches[matches.match == match_choice]
        match_id = matchh.match_id.unique()[0]
        events = pd.DataFrame()
    ...
    if selected_plot == 'Assists':
        ...
        st.write('This plot shows the assist made by the selected team in the selected match.')
