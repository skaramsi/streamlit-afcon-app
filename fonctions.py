...
def assists(df, selected_team):
    ...
    # Vérifie si des Goal Assists existent
    if "pass_goal_assist" in df.columns and df["pass_goal_assist"].notna().any():
        data = df[df["pass_goal_assist"].notna()]
        title = "Goal Assists"
    else:
        data = df[df["pass_shot_assist"].notna()]
        title = "Shot Assists"
...
# calculate the number of passes between each position (using min/ max so we get passes both ways)
   ...
   return  selected_formation,st.pyplot(fig, dpi=100, facecolor= 'black' ,bbox_inches=None)
